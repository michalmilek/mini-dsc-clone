...
export default async function handler(
  ...
  if (req.method === "DELETE") {
    try {
      ...
      const reaction = await db.reactionToDirectMessage.findFirst({
        where: {
      ...
      const conversationReactionDeleteKey = `chat:${reaction.directMessage.conversationId}:reaction:delete`;
